...
import com.microsoft.example.GetHashtag;
import com.microsoft.example.TwitterSpout;
import com.microsoft.example.IgnoreWordsBolt;
import com.microsoft.example.PositiveTweetBolt;
import com.microsoft.example.NegativeTweetBolt;
...
public class TwitterStream {
  ...
  public static StormTopology buildTopology(IBatchSpout spout) throws IOException {
    ...
    //Define the topology:
    //1. spout reads and emits streams of tweets
    //2. GetHashtag emits hashtags pulled from tweets
    ...
    .each(new Fields("tweet"), new GetHashtag(), new Fields("hashtag"))
  ...
  public static void main(String[] args) throws Exception {
    ...
    if(args.length==0) {
      ...
      try {
        local.submitTopology("hashtagTopology", conf, buildTopology(spout));
      } catch (IOException e) {
